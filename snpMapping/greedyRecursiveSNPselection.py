...
def main(args, parser):
    ...
    with open(args.snp, 'r') as input:
        ...
        for l in input:
            s = l.rstrip().split()
            snpObjs[s[3]].append(SNP(s[3], s[0], int(s[4]), s[5], s[6], float(s[1]), float(s[2])))
...
def SNPSelectionWorkflow(fai, snp, number, minimum = 50000):
    chrlens = dict()
    total = 0
    # Read in fasta index file and get chromosome lengths
...
class SNP:
    ...
    def __init__(self, chrom, name, pos, a1, a2, maf, miss):
        self.chrom = chrom
        self.name = name
        self.pos = pos
        self.a1 = a1
    ...
    def calcScore(self, start, end):
        ...
        self.curscore = self.maf * self.call * (length - abs(2 * self.pos - (end + start)))
