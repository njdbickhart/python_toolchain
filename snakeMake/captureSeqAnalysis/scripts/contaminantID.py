...
usage = f'python {sys.argv[0]} <input mash distances> <contaminant name file> <filtered output> <output binary detection> <calibrator depth of coverage>'
...
names = set()
depth = 0.0
with open(sys.argv[5], 'r') as input:
    depth = float(input.readline())
...
with open(sys.argv[1], 'r') as input, open(sys.argv[3], 'w') as filtered, open(sys.argv[4], 'w') as binary:
    ...
    for l in input:
        ...
        if s[4] in names:
            ...
            if kratio > 0.8 and int(s[2]) > depth/3:
                binary.write(f'{s[4]}\n')